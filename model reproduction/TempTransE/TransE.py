...
class TransE(nn.Module):
    ...
    def __init__(self, numOfEntity, numOfRelation, entityDimension, relationDimension, margin, norm, entity_embeddings=None, relation_embeddings=None):
    ...
    def fastValidate(self, validateHead, validateRelation, validateTail):
        ...
        targetLoss = torch.norm(validateHeadEmbedding + validateRelationEmbedding - validateTailEmbedding, self.norm).repeat(self.numOfEntity, 1)#验证三元组的得分
        ...
        tmpHeadLoss = torch.norm(self.entity_embeddings.weight.data + tmpRelationEmbedding - tmpTailEmbedding,
                                 self.norm, 1).view(-1, 1)#将验证三元组的 头实体 替换为所有实体后的得分 （n, 1）
        tmpTailLoss = torch.norm(tmpHeadEmbedding + tmpRelationEmbedding - self.entity_embeddings.weight.data,
                                 self.norm, 1).view(-1, 1)#将验证三元组的 尾实体 替换为所有实体后的得分

        rankH = torch.nonzero(nn.functional.relu(targetLoss - tmpHeadLoss)).size()[0]#验证三元组（h,r,t）在所有替换头三元组（h', r,t）中的排名。排名少了1，所以下面的式子加了2
    ...
    def test_entity_predict(self, Ranks, testHead, testRelation, testTail, trainTriple):
        testHeadEmbedding = self.entity_embeddings(testHead)
        testRelationEmbedding = self.relation_embeddings(testRelation)
        testTailEmbedding = self.entity_embeddings(testTail)
        ...
        Ranks[0] = numOfWrongHead - numOfFilterHead+1
        Ranks[1] = numOfWrongTail - numOfFilterTail+1
        return numOfWrongHead - numOfFilterHead+1, numOfWrongTail - numOfFilterTail+1


    def test_relation_predict(self, testHead, testRelation, testTail, trainTriple):
        testHeadEmbedding = self.entity_embeddings(testHead)
        testRelationEmbedding = self.relation_embeddings(testRelation)
        testTailEmbedding = self.entity_embeddings(testTail)

        targetLoss = torch.norm(testHeadEmbedding + testRelationEmbedding - testTailEmbedding, self.norm).repeat(
            self.numOfRelation, 1)
        ...
        RelationLoss = torch.norm(tmpHeadEmbedding + self.relation_embeddings.weight.data - tmpTailEmbedding, self.norm, 1).view(-1, 1)

        unCorrR = torch.nonzero(nn.functional.relu(targetLoss - RelationLoss))[:, 0]
        ...
        for wrongRelation in unCorrR:
            if trainTriple[(trainTriple[:, 0] == testHead) & (trainTriple[:, 1] == wrongRelation) & (
                    trainTriple[:, 2] == testTail)].size()[0]:
                numOfFilterRelation += 1

        return numOfWrongRelation + 1 -numOfFilterRelation

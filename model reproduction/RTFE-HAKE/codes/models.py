...
class KGEModel(nn.Module, ABC):
    ...
    @staticmethod
    def test_step(model, data_reader, mode, args, is_dynamic=False, time_index=None, head_tail_split=False):
        '''
        ...
        if head_tail_split:
            ...
            length = len(logs)//2
            head_logs = logs[:length]
            tail_logs = logs[length:]
            ...
            return metrics, head_metrics, tail_metrics
...
class HAKE(KGEModel):
    def __init__(self, num_entity, num_relation, hidden_dim, gamma, modulus_weight=1.0, phase_weight=0.5, entity_word_emb=None, relation_word_emb=None):
        ...
        if entity_word_emb != None:
            self.entity_embedding.data[:,:768]=entity_word_emb
        ...
        if relation_word_emb != None:
            self.relation_embedding.data[:,:hidden_dim]=relation_word_emb[:,:hidden_dim]

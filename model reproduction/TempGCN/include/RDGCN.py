...
class RDGCN_layer(torch.nn.Module):
    ...
    def get_loss(self, output_layer, dual_X, KG, negs, num, k, gamma):
        ...
        scores_pos = torch.abs(torch.cat((head_emb, tail_emb), 1) - rel_emb).sum(axis=1)#
        ...
        rel_emb = dual_X.index_select(0, rel_index)
        tail_emb = output_layer.index_select(0, tail_index)
        scores_neg = torch.abs(torch.cat((head_emb, tail_emb), 1) - rel_emb).sum(axis=1)
    ...
    def get_negs(self, KG, output_layer, num, k):
        self.get_k_nearst_neigh(output_layer, k)
        negs=[]
        for tri in KG:
            tri_negs = self.get_tri_negs(tri, num)
            negs.extend(tri_negs)
        return negs


    #get num negetive triplets of a pos triplet
    def get_tri_negs(self, triplet, num):
        tri_negs = []
        for i in range(num):
            while True:
                corrupted_triplet = self.getCorruptedTriplet(triplet)
    ...
    def getCorruptedTriplet(self, triplet):
        ...
        if i < 0:  # 小于0，打坏三元组的头实体
            while True:
                entityTemp = sample(self.rank_k_dict[head], 1)[0]
                if entityTemp != head:
                    break
            corruptedTriplet = (entityTemp, triplet[1], tail)
        else:  # 大于等于0，打坏三元组的尾实体
            while True:
                entityTemp = sample(self.rank_k_dict[tail], 1)[0]
                if entityTemp != tail:
                    break
...
def training(model, primal_X_0, act_func, alpha, beta, gamma, num, k, e, r_num, KG, SKG, learning_rate, epochs):
    ...
    for i in range(epochs):
        ...
        if i % 5 == 0:
            output, dual_X = model(primal_X_0, e,  KG, head, tail, head_r, tail_r, r_mat, M, M_arr, act_func, alpha, beta,
                           gamma, k)
            negs = model.get_negs(KG, output, num, k)

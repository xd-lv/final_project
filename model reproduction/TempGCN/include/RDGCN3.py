...
class RDGCN_layer(torch.nn.Module):
    ...
    def get_negs(self, KG, output_layer, num, k, e_num, r_num):
        #self.get_k_nearst_neigh(output_layer, k)
        negs=[]
        for tri in KG:
            tri_negs = self.get_tri_negs(tri, num, e_num, r_num)
            negs.extend(tri_negs)
        return negs


    #get num negetive triplets of a pos triplet
    def get_tri_negs(self, triplet, num, e_num, r_num):
        tri_negs = []
        for i in range(num):
            while True:
                corrupted_triplet = self.getCorruptedTriplet(triplet, e_num, r_num)
    ...
    def getCorruptedTriplet(self, triplet, e_num, r_num):
        head = triplet[0]
        rel = triplet[1]
        tail = triplet[2]
        i = uniform(0, 1)
        ...
        if i < 0.35:  # 小于0，打坏三元组的头实体
            ...
            while True:
                relTemp = sample(range(r_num), 1)[0]
                if relTemp != rel:
                    break
            corruptedTriplet = (head, relTemp, tail)
...
def training(model, primal_X_0, act_func, alpha, beta, gamma, num, k, e, r_num, KG, SKG, learning_rate, epochs):
    ...
    for i in range(epochs):
        ...
        if i % 5 == 0:
            #output, dual_X = model(primal_X_0, e,  KG, head, tail, head_r, tail_r, r_mat, M, M_arr, act_func, alpha, beta,
            #               gamma, k)
            negs = model.get_negs(KG, output, num, k, e, r_num)

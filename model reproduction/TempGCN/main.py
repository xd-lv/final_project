...
if __name__ == '__main__':
    ...
    #model = torch.load(Config.data + 'static_model.chkpnt', map_location=torch.device('cuda'))
    #embedding_list = get_input_embeddings(Config.data, 'vectorList_TransE_dim150.json')
    embedding_list = get_input_embeddings(Config.data, 'vectorList_RDGCN_xinit.json')
    ...
    #for time in range(last_time):
        ...
        output, dual_X, J = training(model, input_embedding, Config.act_func, Config.alpha, Config.beta, Config.gamma, Config.num, Config.k, e, r_num, KG, all_SKG, 0.001, Config.epochs)

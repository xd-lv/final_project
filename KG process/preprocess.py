...
fr = open(data_path+'all.txt')
numberOfLines = len(fr.readlines())
print('the number of unlist company:'+str(numberOfLines))
fr = open(data_path+'all.txt')
next(fr)
for line in fr.readlines():
    line = line.strip()
    ...
    if len(listFromLine)<14:
        continue
    public_date = listFromLine[13][1:5]

    for node in keyword(listFromLine[11].strip('"')):
        main = []
        main.append(listFromLine[10].strip('"'))
        main.append('company_main')
        main.append(node)
        main.append(public_date)
        company_quad.append(main)

    for node in keyword(listFromLine[12].strip('"')):
        scope = []
        scope.append(listFromLine[10].strip('"'))
        scope.append('company_scope')
        scope.append(node)
        scope.append(public_date)
        company_quad.append(scope)
...
for line in fr.readlines():
    ...
    if len(listFromLine)>3:
        for node in keyword(listFromLine[3].strip('"')):
            digest = []
            digest.append(listFromLine[1].strip('"'))
            digest.append('zhy_patent_digest')
            digest.append(node)
            digest.append(public_date)
            zhy_patent_quad.append(digest)

fr = open(data_path+'unzhy.txt')
numberOfLines = len(fr.readlines())
print('the number of un zhy_patent:'+str(numberOfLines))
fr = open(data_path+'unzhy.txt')
next(fr)
for line in fr.readlines():
    line = line.strip()
...
for line in fr.readlines():
    ...
    '''
    mot_literature = []
    mot_literature.append(listFromLine[1].strip('"'))
    ...
    if len(listFromLine[4])!=0:
        meeting_name = []
        meeting_name.append(listFromLine[1].strip('"'))
    ...
    if len(listFromLine[5]) != 0:
        host_unit = []
        host_unit.append(listFromLine[1].strip('"'))
...
for line in fr.readlines():
    ...
    '''
    periodical_name = []
    periodical_name.append(listFromLine[1].strip('"'))
    ...
    if len(listFromLine) == 5:
        if len(listFromLine[4])!=0:
            ...
            for key in keywords:
                temp = []
                temp.append(listFromLine[1].strip('"'))
...
for line in fr.readlines():
    ...
    if (len(listFromLine) > 3):
        ...
        for type in type_code_ys:
            temp = []
            temp.append(listFromLine[2].strip('"'))
    ...
    if (len(listFromLine) > 4):
        for node in keyword(listFromLine[4].strip('"')):
            digest = []
            digest.append(listFromLine[2].strip('"'))
    ...
    if (len(listFromLine) > 5):
        for node in keyword(listFromLine[5].strip('"')):
            sovereignty = []
            sovereignty.append(listFromLine[2].strip('"'))
...
fr = open(data_path+'unwf.txt')
numberOfLines = len(fr.readlines())
print('the number of un wf_patent:'+str(numberOfLines))
fr = open(data_path+'unwf.txt')
next(fr)
for line in fr.readlines():
    line = line.strip()
    ...
    if (len(listFromLine) > 5):
        for node in keyword(listFromLine[2].strip('"')):
            sovereignty = []
            sovereignty.append(listFromLine[2].strip('"'))
            sovereignty.append('wf_patent_sovereignty')
            sovereignty.append(node)
...
for line in fr.readlines():
    ...
    company_conference.append(listFromLine[4].strip('"'))
...
fr = open(data_path+'unlist_investor.txt')
numberOfLines = len(fr.readlines())
print('the number of unlist company_company:'+str(numberOfLines))
fr = open(data_path+'unlist_investor.txt')
next(fr)
for line in fr.readlines():
    line = line.strip()
...
for line in fr.readlines():
    ...
    company_journal.append(listFromLine[4].strip('"'))
...
for line in fr.readlines():
    ...
    company_wf.append(listFromLine[4].strip('"'))
    company_wf.append(public_date)
    company_wf_quad.append(company_wf)

fr = open(data_path+'unlist_wf.txt')
numberOfLines = len(fr.readlines())
print('the number of unlist company_wf:'+str(numberOfLines))
fr = open(data_path+'unlist_wf.txt')
next(fr)
for line in fr.readlines():
    line = line.strip()
    ...
    company_wf.append(listFromLine[0].strip('"'))
    company_wf.append('company_wf')
    company_wf.append(listFromLine[3].strip('"'))
    company_wf.append(public_date)
    company_wf_quad.append(company_wf)
...
for line in fr.readlines():
    ...
    company_zhy.append(listFromLine[4].strip('"'))
    company_zhy.append(public_date)
    company_zhy_quad.append(company_zhy)

fr = open(data_path+'unlist_zhy.txt')
numberOfLines = len(fr.readlines())
print('the number of unlist company_zhy:'+str(numberOfLines))
fr = open(data_path+'unlist_zhy.txt')
next(fr)
for line in fr.readlines():
    #line = line.strip()
    ...
    company_zhy.append(listFromLine[0].strip('"'))
    company_zhy.append('company_zhy')
    company_zhy.append(listFromLine[3].strip('"'))
    company_zhy.append(public_date)
    company_zhy_quad.append(company_zhy)
...
for line in content:
    if line[2]=='无人驾驶' or line[2]=='半导体材料' or line[2]=='医疗器械':
        continue
    company_industry = []
    company_industry.append(line[0])
    company_industry.append('company_industry')
    if len(line)>3:
        company_industry.append(line[3])
    else:
        company_industry.append(line[2])
...
fr = open(data_path+'all.txt')
numberOfLines = len(fr.readlines())
print('the number of unlisted_company:'+str(numberOfLines))
fr = open(data_path+'all.txt')
next(fr)
for line in fr.readlines():
    line = line.strip()
    ...
    temp = []
    temp.append(listFromLine[10].strip('"'))
    temp.append('company_industry')
    if len(split) > 5:
        if split[1].strip('"') == '网络层':
            temp.append('传输层')
        else:
            temp.append(split[1].strip('"'))
    else:
        temp.append(split[0].strip('"'))
    temp.append(public_date)
    company_industry_quad.append(temp)
